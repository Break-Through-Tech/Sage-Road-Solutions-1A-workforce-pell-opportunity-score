...
### Key Details
* **What's in the data:** IPEDS provides the institution roster (~5,700 Title IV institutions) keyed by UnitID, with location, sector, and awards conferred by CIP code and credential level. College Scorecard adds completion, earnings, and debt outcomes at institution and field-of-study level. BLS OEWS gives employment counts and wages by SOC occupation across ~530 metro and nonmetro areas (current release: May 2025, SOC 2018). O*NET supplies occupation attributes — job zones, required education, skills. Census ACS 5-year estimates provide county-level educational attainment, income, poverty, and labor force participation.

* **Known limitations and preprocessing:** No ground-truth label exists — nothing in these sources flags a program as Workforce Pell aligned, and the two criteria that most drive state approval (job placement rate, value-added earnings) are absent entirely, so a labeling or scoring strategy must be agreed before modeling. Coverage is a further constraint: eligible programs run 150–599 clock hours over 8–15 weeks, and most training in that range is non-credit and therefore invisible in IPEDS Completions, which may push the analytic unit from program to institution. The CIP-to-SOC crosswalk is the load-bearing join and is many-to-many, requiring a documented rule for collapsing multiple matches before feature engineering begins. Geography must also be reconciled — IPEDS gives one campus address, OEWS reports by metro/nonmetro area, ACS by county — and one convention applied consistently. Finally, cell suppression in Scorecard (small certificate programs) and OEWS (small area-occupation pairs) falls hardest on rural institutions, and standardizing on CIP 2020 → SOC 2018 is needed to align taxonomies across sources.

* **Documentation and data dictionaries:** IPEDS — https://nces.ed.gov/ipeds/use-the-data · CIP codes and CIP-to-SOC crosswalk — https://nces.ed.gov/ipeds/cipcode/ · College Scorecard — https://collegescorecard.ed.gov/data/documentation/ · BLS OEWS tables and technical notes — https://www.bls.gov/oes/tables.htm · O*NET database and dictionary — https://www.onetcenter.org/database.html · Census ACS — https://www.census.gov/programs-surveys/acs/data.html
